...
class GCT(object):
	...
	command_string += string.join(["?" for x in range(len(data_array))], ",")
	command_string += ")"
        ...
        c = self._meta.cursor()
        c.execute(command_string, data_array)
        self._meta.commit()
        c.close()
    ...
    def _read_gct(self,src,verbose=True,frame=True):
        ...
        titles = reader.next()
	logger.debug("titles:  {}".format(titles))
	...
	logger.debug("row_meta_headers:  {}".format(row_meta_headers))
	...
	self._add_table_to_meta_db('row', row_meta_headers)
        ...
        while current_row < int(dims[3]):
            ...
            current_row += 1
	logger.debug("col_meta_array:  {}".format(col_meta_array))
    ...
    def read(self,src=None,verbose=True,cid=None,rid=None,
        ...
        it may also be either of the strings 'lm_epsilon' or 'bing', in which case
        the appropriate genes are retrived from Mongo
        '''
        #determine file type
        if not src:
            src = self.src
            ...
            if extension == '.gct':
                ...
                if type(rid) == str:
                    if re.match('.*\.grp$', rid) and os.path.exists(rid):
                        ...
                        genedb = mu.CMapMongo(mongo_location = None,
                                              collection = 'gene_info')
                        rid = genedb.find({'is_lm' : True, 'pr_pool_id' : 'epsilon'},
                                          fields = ['pr_id'])
                    elif rid == 'bing':
                        genedb = mu.CMapMongo(mongo_location = None,
                                              collection = 'gene_info')
                        rid = genedb.find({'is_bing' : True},
                                          fields = ['pr_id'])
                # ditto
                if matrix_only:
...
class fastGCT(object):
    '''
    ***DO NOT USE IN PRODUCTION CODE***
    DOES NOT SUPPORT INDEXING
    use case: if you have a .gctx file with a good deal of metadata that you want
    to load in its entirity. this class just reads the metadata faster.
    after paper publication we'll speed all this up in the actual GCT class,
    this is a speedier workaround in the meantime
    returns an object with "frame", "rdesc", and "cdesc" structures.
    '''
    def __init__(self, src):
        self.src = src
        with tables.openFile(src) as f:
            # get matrix, row id's, col id's
            mat = f.getNode('/0/DATA/0', 'matrix')[:].T.astype(numpy.float)
            rid = [x.strip() for x in f.getNode('/0/META/ROW', 'id')[:]]
            cid = [y.strip() for y in f.getNode('/0/META/COL', 'id')[:]]
            self.frame = pd.DataFrame(mat, index = rid, columns = cid)
            # get row desc
            rhd = [x.name for x in f.listNodes('/0/META/ROW')]
            rdesc = {}
            for field in rhd:
                this_field = f.getNode('/0/META/ROW', field)[:]
                if this_field.dtype.type is numpy.string_:
                    rdesc[field] = [x.strip() for x in this_field]
                else:
                    rdesc[field] = this_field.tolist()
            self.rdesc = pd.DataFrame(rdesc).set_index('id')
            # get column desc
            chd = [y.name for y in f.listNodes('/0/META/COL')]
            cdesc = {}
            for field in chd:
                this_field = f.getNode('/0/META/COL', field)[:]
                if this_field.dtype.type is numpy.string_:
                    cdesc[field] = [y.strip() for y in this_field]
                else:
                    cdesc[field] = this_field.tolist()
            self.cdesc = pd.DataFrame(cdesc).set_index('id')
