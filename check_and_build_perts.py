...
def build_assayplate_pertplate_map(plate_tracking_file):
        ...
        data = pd.read_table(plate_tracking_file, header=0, sep='\t')
        # Return sorted list of all assay plate barcodes
        assay_plates = sorted(set(data['assay_plate_barcode']))
        assayplate_pertplate_map = {}

        # For each assay plate, find its entry and add its pert plate name to the map
        for assay in assay_plates:
                assay_plate_entry = data.loc[data['assay_plate_barcode'] == assay]
                pert_plate = assay_plate_entry['pert_plate']
                assayplate_pertplate_map[assay] = pert_plate.values[0]
...
def write_plate_maps(pert_plate_perturbagens_map):

        def index_builder(pert):
                return str(pert.well_id)
        ...
        for (pertplate) in pert_plate_perturbagens_map:
                well_map = prism_metadata.validate_perturbagens(pert_plate_perturbagens_map[pertplate])
                pert_list = []
                for key in well_map:
                        temp = well_map[key]
                        pert_list.append(temp)
                dataframe = prism_metadata.convert_objects_to_metadata_df(index_builder, pert_list,
                                                           {"well_id": "pert_well"})
                dataframe_indexed = dataframe.set_index("pert_well")
                dataframe_indexed.drop('assay_plate_barcode', axis=1, inplace=True)
                sorted_dataframe = dataframe_indexed.sort_index(axis='index')
                sorted_dataframe.to_csv('{}.src'.format(pertplate), sep='\t')
...
def main(plate_map_path, plate_tracking_file, config_filepath):
        ...
        write_plate_maps(pert_plate_perturbagens_map)